...
{{/*
Add S3 environmental variables if enabled.
*/}}
{{- define "invenio.config.s3" -}}
{{- if or .Values.minio.enabled .Values.minioExternal -}}
{{- if or .Values.minio.enabled .Values.minioExternal.endpointUrl }}
- name: INVENIO_S3_ENDPOINT_URL
  value:  {{ include "invenio.s3.endpoint" }}
{{- end }}
- name: INVENIO_S3_ACCESS_KEY
  valueFrom:
    secretKeyRef:
      name: {{ include "invenio.s3.secretName"}}
      key: {{ include "invenio.s3.accessKeyKey" }}
- name: INVENIO_S3_SECRET_KEY
  valueFrom:
    secretKeyRef:
      name:  {{ include "invenio.s3.secretName"}}
      key: {{ include "invenio.s3.accessKeyIdKey"}}
{{- end }}
{{- end -}}
...
*/}}
{{- define "invenio.s3.accessKeyKey" -}}
  {{- if .Values.minio.enabled  -}}
...
*/}}
{{- define "invenio.s3.accessKeyIdKey" -}}
  {{- if .Values.minio.enabled  -}}
    rootPassword
  {{- else -}}
...
{{- if .Values.invenio.s3FileStorage.enabled -}}
